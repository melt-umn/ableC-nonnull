...
stage ("Build") {
  ...
  node {
    ...
    checkout([ $class: 'GitSCM',
               ...
               extensions: [
                 [ $class: 'RelativeTargetDirectory',
                   relativeTargetDir: 'ableC/edu.umn.cs.melt.exts.ableC.nonnull']
               ],
               submoduleCfg: [],
               userRemoteConfigs: [
                 [url: 'https://github.com/melt-umn/edu.umn.cs.melt.exts.ableC.nonnull.git']
               ]
             ])

    /* env.PATH is the master's path, not the executor's */
    withEnv(["PATH=${SILVER_BASE}/support/bin/:${env.PATH}"]) {
      sh "cd ableC/edu.umn.cs.melt.exts.ableC.nonnull/artifact && ./build.sh"
    }
  }
...
stage ("Modular Analyses") {
  node {
    withEnv(["PATH=${SILVER_BASE}/support/bin/:${env.PATH}"]) {
      def mdir = "ableC/edu.umn.cs.melt.exts.ableC.nonnull/modular_analyses"
...
stage ("Test") {
  node {
    def top_dir = "ableC/edu.umn.cs.melt.exts.ableC.nonnull"
